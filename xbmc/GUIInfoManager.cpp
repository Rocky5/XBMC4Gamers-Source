...
int CGUIInfoManager::TranslateSingleString(const CStdString &strCondition)
{
  ...
  else if (strCategory.Equals("system"))
  {
    ...
    else if (strTest.Equals("system.memory(free_no_mb)")) ret = SYSTEM_FREE_MEMORY_NO_MB;
    ...
    else if (strTest.Equals("system.memory(used_no_mb)")) ret = SYSTEM_USED_MEMORY_NO_MB;
    else if (strTest.Equals("system.memory(used)")) ret = SYSTEM_USED_MEMORY;
...
int CGUIInfoManager::TranslateListItem(const CStdString &info)
{
  ...
  else if (info.Equals("Synopsis_AltName")) return LISTITEM_SYNOPSISALTNAME;
  else if (info.Equals("Synopsis_Overview")) return LISTITEM_SYNOPSISOVERVIEW;
  else if (info.Equals("Synopsis_Developer")) return LISTITEM_SYNOPSISDEVELOPER;
  else if (info.Equals("Synopsis_Publisher")) return LISTITEM_SYNOPSISPUBLISHER;
  else if (info.Equals("Synopsis_Features_General")) return LISTITEM_SYNOPSISFEATURES_GENERAL;
  else if (info.Equals("Synopsis_Features_Online")) return LISTITEM_SYNOPSISFEATURES_ONLINE;
  else if (info.Equals("Synopsis_ESRB")) return LISTITEM_SYNOPSISESRB;
  else if (info.Equals("Synopsis_ESRB_Descriptors")) return LISTITEM_SYNOPSISESRB_DESCRIPTORS;
  else if (info.Equals("Synopsis_Genre")) return LISTITEM_SYNOPSISGENRE;
  else if (info.Equals("Synopsis_Release_Date")) return LISTITEM_SYNOPSISRELEASE_DATE;
  else if (info.Equals("Synopsis_Year")) return LISTITEM_SYNOPSISYEAR;
  else if (info.Equals("Synopsis_Rating")) return LISTITEM_SYNOPSISRATING;
  else if (info.Equals("Synopsis_Platform")) return LISTITEM_SYNOPSISPLATFORM;
  else if (info.Equals("Synopsis_Exclusive")) return LISTITEM_SYNOPSISEXCLUSIVE;
  else if (info.Equals("Synopsis_TitleID")) return LISTITEM_SYNOPSISTITLEID;
  ...
  case SYSTEM_TOTAL_MEMORY_NO_MB:
    {
      ...
      else if (info == SYSTEM_FREE_MEMORY_NO_MB)
        strLabel.Format("%i", stat.dwAvailPhys /MB);      
      ...
      else if (info == SYSTEM_USED_MEMORY_NO_MB)
        strLabel.Format("%i", (stat.dwTotalPhys - stat.dwAvailPhys)/MB);
...
CStdString CGUIInfoManager::GetItemLabel(const CFileItem *item, int info)
{
  ...
  case LISTITEM_LABEL2:
   ...
   case LISTITEM_SYNOPSISALTNAME:
	 return item->GetLabelSynopsis_AltName();
   case LISTITEM_SYNOPSISOVERVIEW:
	 return item->GetLabelSynopsis_Overview();
   case LISTITEM_SYNOPSISDEVELOPER:
	 return item->GetLabelSynopsis_Developer();
   case LISTITEM_SYNOPSISPUBLISHER:
	 return item->GetLabelSynopsis_Publisher();
   case LISTITEM_SYNOPSISFEATURES_GENERAL:
	 return item->GetLabelSynopsis_FeaturesGeneral();
   case LISTITEM_SYNOPSISFEATURES_ONLINE:
	 return item->GetLabelSynopsis_FeaturesOnline();
   case LISTITEM_SYNOPSISESRB:
	 return item->GetLabelSynopsis_ESRB();
   case LISTITEM_SYNOPSISESRB_DESCRIPTORS:
	 return item->GetLabelSynopsis_ESRBDescriptors();
   case LISTITEM_SYNOPSISGENRE:
	 return item->GetLabelSynopsis_Genre();
   case LISTITEM_SYNOPSISRELEASE_DATE:
	 return item->GetLabelSynopsis_ReleaseDate();
   case LISTITEM_SYNOPSISYEAR:
	 return item->GetLabelSynopsis_Year();
   case LISTITEM_SYNOPSISRATING:
	 return item->GetLabelSynopsis_Rating();
   case LISTITEM_SYNOPSISPLATFORM:
	 return item->GetLabelSynopsis_Platform();
   case LISTITEM_SYNOPSISEXCLUSIVE:
	 return item->GetLabelSynopsis_Exclusive();
   case LISTITEM_SYNOPSISTITLEID:
	 return item->GetLabelSynopsis_TitleID();
   case LISTITEM_SYNOPSISFANART:
	 return item->GetLabelSynopsis_Fanart();
   case LISTITEM_SYNOPSISRESOURCES:
