...
#endif
		{
			CStdString seperator;
			g_charsetConverter.stringCharsetToUtf8("ISO-8859-1", "\u00B7", seperator);
			CStdString wszText;
			MEMORYSTATUS stat;
			GlobalMemoryStatus(&stat);
...
//          CGUIMessage message(GUI_MSG_NOTIFY_ALL, g_windowManager.GetActiveWindow(), 0, GUI_MSG_UPDATE_ITEM, 1, msgItem); // 1 to update the listing as well 
	...
	{  
		...
		XFILE::CFile::Cache("Q:/system/backups/filezilla server.xml","Q:/system/filezilla server.xml");
		// XFILE::CFile::Cache("Q:/system/backups/profiles.xml","Q:/system/userdata/profiles.xml");
